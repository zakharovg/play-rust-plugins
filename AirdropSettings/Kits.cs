...
namespace Oxide.Plugins
{
    ...
    class Kits : RustPlugin
    {
        ...
        void TryGiveKit(BasePlayer player, string kitname)
        {
            ...
            SendReply(player, "Набор "+kitname+" получен");
        ...
        {
            Kit kit;
            if (!storedData.Kits.TryGetValue(kitname, out kit)) return "Такого набора нет";

            object thereturn = Interface.Oxide.CallHook("canRedeemKit", player);
            if (thereturn != null)
            {
                if (thereturn is string) return thereturn;
                return "Вы не можете сейчас получить этот набор";
            }

            if (kit.authlevel > 0)
                if (player.net.connection.authLevel < kit.authlevel)
                    return "У вас недостаточно прав для получения этого набора";
            ...
            if (kit.max > 0)
                if (kitData.max >= kit.max)
                    return "Вы уже получили все доступные наборы";

            if (kit.cooldown > 0)
            {
                var ct = CurrentTime();
                if (kitData.cooldown > ct && kitData.cooldown != 0.0)
                    return $"Вам нужно подождать {Math.Abs(Math.Ceiling(kitData.cooldown - ct))} секунд прежде чем снова получать этот набор";
            }
            ...
            if (kit.npconly)
            {
                ...
                if (!foundNPC)
                    return "Вам нужно найти NPC который выдает этот набор.";
            }
            return true;
        }
        ...
        private const string buttonjson = @"[
			...
			},
			{
				...
				[
					{
						""type"":""UnityEngine.UI.Text"",
						""text"":""Нажмите кнопку чтобы получить"",
						""fontSize"":15,
						""align"": ""MiddleCenter"",
                    },
                    {
        ...
        void cmdChatKit(BasePlayer player, string command, string[] args)
        {
            if (args.Length == 0)
            {
                ...
                else
                {
					...
					SendReply(player, "Продвинутые наборы можно купить в нашем магазине по адресу:\n <color=#FFCC33>rustld.playrust.ru</color>");
                }
                return;
            }
            if (args.Length == 1)
