...
namespace Oxide.Plugins {
    ...
    public class Clans : RustPlugin {
		...
		// REPLACE THIS BLOCK OF CODE FOR CLANSUI
        #region Plugin API
		...
		[HookMethod("ForceJoinClan")]
		private bool ForceJoinClan(string tag, ulong playerId)
		{
			if (string.IsNullOrEmpty(tag) || playerId == default(ulong))
				return false;
			...
			var playerIdString = playerId.ToString(CultureInfo.InvariantCulture);
			if (clan.members.Contains(playerIdString))
				return false;

			clan.members.Add(playerIdString);
			SaveData();
			lookup[playerIdString] = clan;
			var player = BasePlayer.FindByID(playerId);
			if (player != null)
			{
				SetupPlayer(player);
			...
			var playerData = Interface.Oxide.GetLibrary<Covalence>().Players.GetPlayer(playerIdString);
			var playerNickName = player == null ? playerData.Nickname : player.displayName;

			clan.Broadcast(_("%NAME% has joined the clan!", new Dictionary<string, string> { { "NAME", StripTag(playerNickName, clan) } }));
			foreach (var memberId in clan.members)
			{
				if (memberId != playerIdString && IsInstalled() && addClanMatesAsFriends)
				{
					AddFriend(memberId, playerIdString);
					AddFriend(playerIdString, memberId);
				}
			}
			return true;
		}
