...
namespace Oxide.Plugins
{
	...
	public class Clans : RustPlugin
	{
		...
		private List<string> texts = new List<string>() {
            ...
            "You don't have enough tuna to create a clan. Required amount: ",
		...
		private bool ForceJoinClan(string tag, BasePlayer player)
		{
			if (string.IsNullOrEmpty(tag) || player == null)
				return false;
			...
			lookup[userId] = clan;
			SetupPlayer(player);
			clan.Broadcast(_("%NAME% has joined the clan!", new Dictionary<string, string> { { "NAME", StripTag(player.displayName, clan) } }));
