...
{
    [Info("LustyMap", "Kayzor", "1.0.20", ResourceId = 1333)]
    ...
    public class LustyMap : RustPlugin
    {
        ...
        void Loaded()
        {
            ...
            set("LustyMap", "MapURL", "http://185.38.148.8:28025/map.jpg", false);
            set("LustyMap", "MapComplexURL", "http://lustyrust.co.uk/img/lustyplugins/map/", false);
        ...
        // Add new players to Minimap list (broken)
        ...
        //        if (startopen)
        //        {
        //            timer.In(10, () => minimapAdd(player));
        //        }
        //        else
        //        {
        //            timer.In(10, () => minimapHUD(player));
        //        }
        //    }
        //}
        ...
        private void mapGUI(BasePlayer player)
        {
            ...
            lustyGUI gui = new lustyGUI();
            gui.add("MapGUI", true, "0.25 0.15", "0.75 0.95", "0 0 0 1");
        ...
        private void minimapGUI(BasePlayer player)
        {
            ...
            if (mapmode)
            {
                ...
                //gui.box("Minimap", "PlayerOut", (mapX - 0.0125f).ToString() + " " + (mapZ - 0.0125f).ToString(), (mapX + 0.0125f).ToString() + " " + (mapZ + 0.0125f).ToString(), "1 1 1 1");
                //gui.box("Minimap", "PlayerIn", (mapX - 0.0100f).ToString() + " " + (mapZ - 0.0100f).ToString(), (mapX + 0.0100f).ToString() + " " + (mapZ + 0.0100f).ToString(), "0 0.5 0 1");

                //gui.url("Minimap", "Self", "http://map.playrust.io/img/self.png", (mapX - 0.025f).ToString() + " " + (mapZ - 0.025f).ToString(), (mapX + 0.025f).ToString() + " " + (mapZ + 0.025f).ToString());

                if (compass)
                {
                    gui.text("Minimap", "Location", "MiddleCenter", "<size=12>" + txtCpsHead + " " + direction + "\n" + player.transform.position.ToString() + "</size>", "0 -0.2", "1 0", "1 1 1 0.8");
                }
        ...
        private class lustyGUI
        {
            ...
            private string buttoncmd = @"""command"":""{command}"",";
            private string buttontext = @"{""parent"": ""{name}{rand}"",""components"":[{""type"":""UnityEngine.UI.Text"",""text"":""{text}"",""fontSize"":16,""align"": ""{align}""},{""type"":""RectTransform"",""anchormin"": ""0 0"",""anchormax"": ""1 1""}]}";
            private string urlblock = @"{""type"":""UnityEngine.UI.RawImage"",""imagetype"": ""Tiled"",""color"": ""0.7 0.7 0.7 {alpha}"",""url"": ""{url}""}";
            private string cursorblock = @"{""type"":""NeedsCursor""}";
            private string endblock = "]}";
