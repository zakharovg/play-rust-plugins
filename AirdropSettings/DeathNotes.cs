...
namespace Oxide.Plugins
{
    ...
    class DeathNotes : RustPlugin
    {
        ...
        Dictionary<ulong, bool> showAnimals = new Dictionary<ulong, bool>();
        ...
        void Loaded()
        {
            ...
            foreach (BasePlayer player in BasePlayer.activePlayerList)
			{
				...
				if (!showAnimals.ContainsKey(player.userID) && HasPerm(player.userID, "see"))
                    showAnimals.Add(player.userID, true);
			}
        ...
        void OnPlayerInit(BasePlayer player)
        {
			...
			if(!showAnimals.ContainsKey(player.userID))
			{
				showAnimals.Add(player.userID, true);
				SaveData();
			}
        ...
        void LoadData()
        {
            ...
            showAnimals = Interface.Oxide.DataFileSystem.ReadObject<Dictionary<ulong, bool>>("DeathNotes-ShowAnimals");
        }

        void SaveData()
        {
            Interface.Oxide.DataFileSystem.WriteObject("DeathNotes", canRead);
            Interface.Oxide.DataFileSystem.WriteObject("DeathNotes-ShowAnimals", showAnimals);
        }
        ...
        void LoadMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string>
            {
				...
				{"AnimalsHidden", "You do no longer see death messages."},
                {"AnimalsUnhidden", "You will now see death messages."}
            }, this);
        }
		...
		[ChatCommand("showanimals")]
        void cmdShowAnimals(BasePlayer player, string cmd, string[] args)
        {
            if(!HasPerm(player.userID, "see"))
            {
                SendChatMessage(player, GetMsg("No Permission", player.userID));
            ...
            if (showAnimals.ContainsKey(player.userID))
            {
                if (showAnimals[player.userID])
                {
                    showAnimals[player.userID] = false;
                    SendChatMessage(player, GetMsg("AnimalsHidden", player.userID));
                }
                else
                {
                    showAnimals[player.userID] = true;
                    SendChatMessage(player, GetMsg("AnimalsUnhidden", player.userID));
                }
            }
            else
            {
        ...
        void OnEntityDeath(BaseCombatEntity victim, HitInfo info)
        {
			...
			if (data.reason == DeathReason.Suicide)
				return;
            ...
            foreach (BasePlayer player in BasePlayer.activePlayerList)
            {
				...
				bool playerCanRead;
				bool playerShowAnimals;
				
				canRead.TryGetValue(player.userID, out playerCanRead);
				showAnimals.TryGetValue(player.userID, out playerShowAnimals);

				if (data.reason != DeathReason.Animal && data.reason != DeathReason.AnimalDeath)
				{
					if (InRadius(player, data.attacker.entity) && playerCanRead)
						SendChatMessage(player, GetDeathMessage(newData, false), null, GetConfig("76561198077847390", "Settings", "Chat Icon (SteamID)"));
				}
				else 
				{
					if (InRadius(player, data.attacker.entity) && playerShowAnimals)
						SendChatMessage(player, GetDeathMessage(newData, false), null, GetConfig("76561198077847390", "Settings", "Chat Icon (SteamID)"));
				}
			}
        ...
        string GetDeathMessage(DeathData data, bool console)
        {
            ...
            if (console)
                ...
                message = GetConfig("{Message}", "Settings", "Formatting").Replace("{Title}", $"<color={GetConfig("#80D000", "Settings", "Title Color")}>{GetConfig("Death Notes", "Settings", "Title")}</color>").Replace("{Message}", $"<color={GetConfig("#696969", "Settings", "Message Color")}>{messages[UnityEngine.Random.Range(0, messages.Count - 1)].ToString()}</color>");
